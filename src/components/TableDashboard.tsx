...
function TableDashboard({ filteredData, emailLogs, onResend, onUpdateKitAndSnack, onUpdateHeavyMeal, onUpdateMissionCard, onEdit, onDelete }: TableDashboardProps) {
          ...
          {filteredData.map((participant, idx) => (
            <tr key={idx} className="hover:bg-white/5 transition-colors duration-200">
              <td className="px-6 py-4 whitespace-nowrap text-gray-400">
                {participant.registered_at ? new Date(participant.registered_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase() : 'N/A'}
              </td>
              ...
              <td className="px-6 py-4 whitespace-nowrap font-medium text-white">
                <div className="flex items-center gap-2">
                  ...
                  {hasSuccessfulEmail(participant.unique) && (
                    <span className="flex h-2 w-2 rounded-full bg-green-500 ring-2 ring-green-500/30" title="Email sent successfully"></span>
                  )}
                </div>
              </td>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-center">
                {participant.present ? (
                  <label className="inline-flex items-center gap-2 cursor-pointer group">
                    ...
                    <span className={`text-xs font-medium transition-colors ${participant.seminar_kit && participant.consumption ? 'text-purple-300' : 'text-gray-500 group-hover:text-gray-400'}`}>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-center">
                {participant.present ? (
                  <label className="inline-flex items-center gap-2 cursor-pointer group">
                    ...
                    <span className={`text-xs font-medium transition-colors ${participant.heavy_meal ? 'text-orange-300' : 'text-gray-500 group-hover:text-gray-400'}`}>
